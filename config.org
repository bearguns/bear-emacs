...
Popular Theme. For me, I want something that is easy to look at for
long periods of time, and minimally colors the source code in which
I'm working. [[https://github.com/nickav/naysayer-theme.el][The naysayer-theme]] checks most of my boxes, and is
inspired by the personal theme used by a programming hero of mine,
Jonathan Blow.
...
The [[https://langserver.org/][Language Server Protocol]], which powers IDE-like features in
a growing number of editors, is awesome. This block configures the
recommended [[https://github.com/emacs-lsp/lsp-mode][lsp-mode]] so that I can take advantage of the myriad
features LSP brings to the table.

I also register various other major modes to work with lsp-mode, even
though part of me thinks the configuration blocks for the individual
modes should handle that part.
...
(defun my/org-tangle-on-config-save ()
  ...
  (when (string= buffer-file-name (file-truename "~/bear-configs/config.org"))
    (org-babel-tangle)
