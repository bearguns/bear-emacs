...
#+END_SRC
*** Modeline
The  Emacs modeline  is a  helpful status  line at  the bottom  of the
buffer view. By  default, it displays the name of  the current buffer,
and  lists active  major and  minor modes.  While I  don't like  going
overboard  with modeline  customization,  the [[https://gitlab.com/jessieh/mood-line][mood-line]]  package is  a
nice, minimal  modeline package  that seems to  focus on  only putting
/helpful/ things in your modeline.
