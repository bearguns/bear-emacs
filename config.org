...
I'm working. The doom-themes package contains a number of themes that
fit this criteria, and let me change things up every now and again easily.
#+BEGIN_SRC emacs-lisp
...
bind the =ace-window= command to =M-o=, since that's bound by default
to set font faces, something I don't do nearly as often (if ever)
compared to moving between windows.
#+BEGIN_SRC emacs-lisp
